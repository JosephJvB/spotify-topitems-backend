...
import { HttpFailure, HttpSuccess, IAudioFeaturesResponse } from '../models/responses'
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if (!trackIds || !trackIds.length) {
      const msg = 'Request is missing "trackIds" parameter'
      console.warn(msg)
    ...
    const ts = Date.now()
    const audioFeatures = await spotifyClient.getAudioFeatures(spotifyToken, trackIds)
    ...
    return new HttpSuccess<IAudioFeaturesResponse>({
      message: 'AudioFeature success',
      token: newToken,
      audioFeatures,
    })
  } catch (e) {
    console.error(e)
    console.error('spotifyAudioFeatures.handler failed')
